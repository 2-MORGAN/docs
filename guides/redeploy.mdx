...
<Warning>
  Le framework est détecté automatiquement depuis les topics GitLab. Si aucun topic `go`/`nextjs`/`nestjs`/`springboot`/`django` n'est trouvé, le redéploiement échoue avec `400 Bad Request`.
</Warning>
